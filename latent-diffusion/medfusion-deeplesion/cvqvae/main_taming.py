...
from modules_taming import Model
from dataset import *
...
def train(data_loader, model, optimizer, args, writer, data_variance=1):
    ...
    for images in data_loader:
        ...
        #writer.add_scalar('loss/train/reconstruction', torch.mean(loss_recons).item(), args.steps)
        #writer.add_scalar('loss/train/quantization', torch.mean(loss_vq).item(), args.steps)
        #writer.add_scalar('loss/train/perplexity', torch.mean(perplexity).item(), args.steps)
...
def test(data_loader, model, args, writer):
    ...
    #writer.add_scalar('loss/test/reconstruction', torch.mean(loss_recons).item(), args.steps)
    #writer.add_scalar('loss/test/quantization', torch.mean(loss_vq).item(), args.steps)
    
    return loss_recons.item(), loss_vq.item()
    #return torch.mean(loss_recons).item(), torch.mean(loss_vq).item()
...
def main(args):
    ...
    dataset_train = Dataset(os.path.join(args.data_folder, 'train'), nu=512, nv=512, transform=transform)
    train_loader = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    
    dataset_val = Dataset(os.path.join(args.data_folder, 'val'), nu=512, nv=512, transform=transform)
    valid_loader = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    ddconfig = {
      ...
      'resolution': 512, #input image size
      'in_channels': num_channels,
      'out_ch': num_channels,
      'ch': args.ch, # channel_size at initial conv
      'ch_mult': [ 1,1,2,2,4],  # num_down = len(ch_mult)-1
      'num_res_blocks': 2,
      'attn_resolutions': [16],
      'dropout': 0.0}
      
    model = Model(ddconfig, args.num_embedding, args.embedding_dim, args.commitment_cost, args.distance,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ch', type=int, default=128, help='initial channel number of first conv (default: 128)')
