...
class DiffusionPipeline(BasicModel):
    def __init__(self, 
        ...
        num_samples = 4,
        do_input_centering = False, # Only for training
        ...
        with torch.no_grad():
            if latent_embedder is not None:
                ...
                self.latent_embedder_checkpoint = torch.load(latent_embedder_checkpoint)
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        if self.latent_embedder is not None:
            ...
            with torch.no_grad():
                ...
                x_0, _, _, _ = self.latent_embedder.encode(x_0)
                
        x_0 = self.scale_factor * x_0
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            cond_img1 = cond_img[:,0,:,:]
            # cond_img2 = cond_img[:,1,:,:]
            def depth2batch(image):
                return (image if image.ndim<5 else torch.swapaxes(image[0], 0, 1))
            images = depth2batch(sample_img)[:32]
            cond_images1 = depth2batch(cond_img1)[:32]
            # cond_images2 = depth2batch(cond_img2)[:32]
            save_image(images, path_out/f'sample_{log_step}.png', normalize=True)
            save_image(cond_images1, path_out/f'condition1_{log_step}.png', normalize=True)
            # save_image(cond_images2, path_out/f'condition2_{log_step}.png', normalize=True)
